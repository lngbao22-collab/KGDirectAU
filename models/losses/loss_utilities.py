...
def _weighted_mean(values: torch.Tensor, weights: torch.Tensor | None) -> torch.Tensor:
    if weights is None:
        return values.mean()
    weights = weights.to(values.device).reshape(-1)
    return (values * weights).sum() / weights.sum().clamp_min(1e-12)


def compute_adversarial_negsamp_losses_chunked(
    ...
    *,
    ...
    pos_loss = -F.logsigmoid(pos_scores)
    loss_parts = [_weighted_mean(pos_loss / 2.0, weights)]

    for start in range(0, num_neg, chunk_size):
        end = min(start + chunk_size, num_neg)
        neg_chunk = score_neg_columns(start, end)
        neg_loss = -(neg_weights[:, start:end] * F.logsigmoid(-neg_chunk)).sum(dim=1)
...
def compute_standard_negsamp_losses_chunked(
    ...
    *,
    ...
    """Standard negative-sampling BCE split into a positive term and chunked negative terms."""
    ...
    pos_loss = -F.logsigmoid(pos_scores)
    loss_parts = [_weighted_mean(pos_loss, weights)]

    for start in range(0, num_neg, chunk_size):
        end = min(start + chunk_size, num_neg)
        neg_chunk = score_neg_columns(start, end)
        if offset != 0.0:
            neg_chunk = neg_chunk + offset
        neg_loss = -F.logsigmoid(-neg_chunk).sum(dim=1)
        loss_parts.append(_weighted_mean(neg_loss, weights))

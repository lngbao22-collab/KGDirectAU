...
def build_model(args) -> nn.Module:
	...
	model = DistMultEncoder(len(entity_dict), len(relation_to_idx), args)
	model.rel_to_idx = relation_to_idx
	return model


class DistMultEncoder(nn.Module):
	"""DistMult encoder that exposes AU-friendly query and entity embeddings."""
	...
	def get_queries_targets(self, src, rel, dst) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
		...
		h = self.ent_embed(src)
		r = self.rel_embed(rel)
		t = self.ent_embed(dst)
		q = h * r
		return q, t, h
